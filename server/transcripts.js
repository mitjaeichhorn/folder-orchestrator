...
export function topicFromRecord (o) {
  let raw = null
  if (o.type === 'last-prompt' && o.lastPrompt) raw = textOf(o.lastPrompt)
  else if (o.type === 'attachment') {
    const a = o.attachment
    if (a?.type === 'queued_command' && a.commandMode === 'prompt') raw = textOf(a.prompt)
  }
  // plumbing must never become a topic, or every action after a /compact is
  // filed under "<command-name>/compact"
  return raw !== null && isOperatorPrompt(raw) ? raw : null
}

/**
 * Claude Code's own plumbing, which arrives on the same `user` records as real
 * prompts and is not something the operator typed.
 *
 * `/compact` alone writes four: the command name, a caveat, the command's
 * stdout, and the continuation preamble the next session opens with. Rendering
 * those as prompts put raw XML in the feed and, worse, made them topics — so
 * every action afterwards was filed under `<command-name>/compact`.
 *
 * Matched on the wrapper tags rather than the text inside them: the tags are
 * structure and stable, the prose is not.
 */
const PLUMBING = [
...
/** Did the operator actually type this, or did Claude Code write it to itself? */
export function isOperatorPrompt (text) {
  const t = String(text ?? '').trim()
  if (!t) return false
  return !PLUMBING.some(re => re.test(t))
}
...
export function parseLine (line, folderId) {
  ...
  if (o.type === 'user' && typeof o.message?.content === 'string') {
    ...
    if (!isOperatorPrompt(text)) return { skip: 'plumbing' }
    if (sessionId) topics.set(sessionId, text.trim().split('\n')[0].slice(0, TOPIC_MAX))
    return { events: [{ folderId, ts, kind: 'prompt', path: null, actor: 'claude', sessionId,
                        tool: null, topic: topicOf(sessionId),
                        // full text, not a 200-char slice: the feed shows the whole
                        // prompt now. Still capped — a pasted wall of text would ride
                        // along in every SSE backfill.
                        detail: { text: text.slice(0, MAX_STR), truncated: text.length > MAX_STR } }] }
  }
